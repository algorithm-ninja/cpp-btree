...
// btree::swap method, which we don't want. And we can't say ::swap because
// then MSVC won't pickup any std::swap() implementations. We can't just use
// std::swap() directly because then we don't get the specialization for string
// and Cord (which are not defined in the std namespace!). So the solution is
// to have a special swap helper function whose name doesn't collide with other
// swap functions defined by the btree classes.
template <typename T>
